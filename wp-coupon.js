jQuery(document).ready(function(){
	vp_set_preview_font();
	jQuery("#couponthumbs img").bind("click", vp_set_preview);
	...
	jQuery("#previewbutton").bind("click", vp_preview_coupon);
	jQuery("#savebutton").bind("click", vp_save_coupon);
...
function vp_toggle_deletion(e) {
	if(this.checked) {
		...
		jQuery("#savebutton").val("Delete coupon");
	} else {
...
function vp_preview_coupon(e) {
	var form = jQuery("#couponform");
	form.attr("action", form.attr("action") + "&preview=coupon");
	form.attr("target", "_blank");
	form.submit();
}
function vp_save_coupon(e) {
	var form = jQuery("#couponform");
	form.attr("action", form.attr("action").replace("&preview=coupon", ""));
	form.attr("target", "");
	form.submit();
}
function vp_set_preview(e) {
	var id = this.id.replace("template_", "");
	var preview = "url(" + vp_siteurl + "/wp-content/plugins/wp-coupon/templates/" + id + "_preview.jpg)";
	jQuery("#couponpreview").css("background-image", preview);
...
function vp_set_preview_font(e) {
	...
	jQuery("#couponpreview h2 textarea").attr("class", font);
	jQuery("#couponpreview p textarea").attr("class", font);
	jQuery("#couponpreview p").attr("class", font);
}
